...
def create_table(metadata: dict, table_name: str, columns: list) -> dict:
    ...
    столбцов ей соответсвует. Также проверяет, что типы данных заданы верно. Если всё
    нормально, то добавляет такую новую таблицу в текущие метаданные.
    '''
...
def drop_table(metadata: dict, cache, table_name: str) -> dict:
    ...
    Функция получает метаданные и название таблицы для удаления на вход,
    и, если такая таблица существует, то удаляет её - иначе выводит ошибку.
    '''
    table_name_clean = table_name.strip().lower()
    ...
    if table_name_clean in metadata.keys():
        ...
        if os.path.exists(table_current_path):
            ...
            print(f'Предупреждение: файл таблицы не найден. Удаление упоминания таблицы из файла "{META_FILE}".') # noqa: E501
